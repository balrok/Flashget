...
def updateConfig(newConfig):
    global config
    for k in newConfig:
        if k in ['cache_dir', 'cache_dir_for_flv', 'flash_dir']:
...
def loadConfig():
    global config
    configFiles = [os.path.join('etc', 'flashget.cfg'), os.path.expanduser(os.path.join('~', '.flashget.cfg')), '.flashget.cfg']
...
def createConfigFile(path):
    ...
    config.set('', '; cache for html pages')
    config.set('', 'cache_dir', os.path.join('~', 'flashget', 'cacheHtml'))
    config.set('', '; cache for the large streamdownloads')
    ...
    config.set('', '; default url from where to start - let it be ;)')
    config.set('', 'link', None)
