...
log = logging.getLogger('urlDownloadRequests')

def void(*args):
    return None

# writes data,redirection into cache
class UrlMgrRequests(object):
...
class LargeDownloadRequests(UrlMgrRequests, threading.Thread):
    uids = 0
    STATE_ERROR = 1
    STATE_FINISHED = 2
    ...
    def __init__(self, *args, **kwargs):
        ...
        UrlMgrRequests.__init__(self, args)
        self.timeout = 120
        ...
        self.uid = LargeDownloadRequests.uids # TODO: we should push to queue (id, key:value) then this can be later used for multiprocessing
        LargeDownloadRequests.uids += 1
        self.state = 0
    ...
    def run(self):
        ...
        if self.downloaded > 0:
            if self.size == self.downloaded:
                self.state = LargeDownloadRequests.STATE_ALREADY_COMPLETED | LargeDownloadRequests.STATE_FINISHED
                ...
                if self.got_requested_position():
                    ...
                    self.state |= LargeDownloadRequests.STATE_DOWNLOAD_CONTINUE
                    if self.megavideo:
        ...
        self.state |= LargeDownloadRequests.STATE_DOWNLOAD
        if stream is None:
        ...
        if not self.request:
            ...
            self.state = LargeDownloadRequests.STATE_ERROR
            self.queue.put(self.uid)
            return
        ...
        while self.downloaded < self.size:
            ...
            if not data_block:
                ...
                block_size = LargeDownloadRequests.best_block_size(after - before, data_block_len)
                self.queue.put(self.uid)
        ...
        if (self.downloaded) != self.size:
            ...
            self.queue.put(self.uid)
            return
        self.state = LargeDownloadRequests.STATE_FINISHED
        self.queue.put(self.uid)
